...
def generate_interview_report_pdf(
    ...
    Includes: score summary, emotion analysis, per-question breakdown.
    """
    ...
    story.append(Paragraph("InterviewAI — Candidate Evaluation Report", title_style))
    story.append(Paragraph(
        f"{candidate_name}  ·  {job_title}{f' at {company}' if company else ''}  ·  {date_str}",
        sub_style
    ))
    ...
    for i, q_obj in enumerate(questions):
        ...
        sc_color     = GREEN if correct else RED
    ...
    story.append(Paragraph(
        f"Generated by InterviewAI  ·  {completed_at[:19] if completed_at else ''}",
        small_style
    ))
